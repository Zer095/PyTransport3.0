...
fileMPP = 'Data/NB_MPP.csv'
fileTrans = 'Data/NB_PyT.csv'
...
with open(fileTrans, mode='r') as file:
    csvFile = csv.reader(file)
    for line in csvFile:
        i = np.where(nbs == float(line[0]))[0][0]
        j = np.where(tol == abs(float(line[1])))[0][0]
...
plt.grid()
plt.savefig('plots/SF_GT_2pt.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig('plots/SF_GT_3pt.pdf', format='pdf',bbox_inches='tight')
...
# plt.savefig('plots/SF_DeltaPz.pdf', format='pdf',bbox_inches='tight')
...
# plt.savefig('plots/SF_DeltafNL.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig('plots/SF_Time_Pz.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig('plots/SF_Time_fNL.pdf', format='pdf',bbox_inches='tight')
...
for i in range(3):
    for j in range(2):
        ...
        axs[i,j].text(-8, 10**-11, f'NB = {nbs[k]}', fontsize=10, bbox=dict(facecolor='lightgray', edgecolor='black', boxstyle='round,pad=0.5'))
        axs[i,j].set_yscale('log')
...
plt.savefig('plots/SF_DeltaPz_alt.pdf', format='pdf',bbox_inches='tight')
...
for i in range(3):
    for j in range(2):
        ...
        axs[i,j].hlines(y=threshold, xmin=-18, xmax=-5, linestyle=(0, (10, 5)), color='black')
        axs[i,j].text(-14, 10**-1, f'NB = {nbs[k]}', fontsize=10, bbox=dict(facecolor='lightgray', edgecolor='black', boxstyle='round,pad=0.5'))
        axs[i,j].set_yscale('log')
        axs[i,j].set_xlim(-18,-5)
        axs[i,j].tick_params(axis='both', direction='in', labelsize=8)
        axs[i,j].grid()
...
plt.savefig('plots/SF_DeltafNL_alt.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig(f'plots/SF_DeltaPz_{NB}.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig(f'plots/SF_2ptTime_{NB}.pdf', format='pdf',bbox_inches='tight') 
...
plt.hlines(y=threshold, xmin=-18, xmax=-5, linestyle=(0, (10, 5)), color='black')
plt.vlines(x=goldtol3M[NB_index] + 0.3, ymin = 10**-14, ymax = 10**-0, linestyle='dashed', color=clr[0])
plt.vlines(x=goldtol3T[NB_index] + 0., ymin = 10**-14, ymax = 10**-0, linestyle='solid', color=clr[1])
plt.xlabel(r'$\epsilon$', fontsize=labsz)
plt.xticks(fontsize=ticsz)
plt.xlim(left=-18, right=-5)
plt.yticks(fontsize=ticsz)
...
plt.grid()
plt.savefig(f'plots/SF_DeltafNL_{NB}.pdf', format='pdf',bbox_inches='tight')
...
plt.legend(fontsize=legsz, framealpha=1.0)
plt.grid()
plt.savefig(f'plots/SF_3ptTime_{NB}.pdf', format='pdf',bbox_inches='tight')
#######################################################################################################################################
plt.show()