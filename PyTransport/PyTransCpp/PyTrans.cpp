 ...
 * @date Fri Dec 13 16:05:07 2024 (Last modified based on provided file)
 *
 ...
 *
 * @section new_features New Features in PyTransport 3.0
 ...
 * The primary functions in this file implement the MPP approach and extend the functionality
 * of PyTransport to compute primordial correlators for multi-field inflationary models:
 * - **MT_backEvolve**: Evolves the background fields and velocities.
 * - **MT_sigEvolve**: Computes the two-point correlation function (sigma).
 * - **MT_alphaEvolve**: Evolves the three-point correlation function (alpha).
 * - **MT_MPP2**: Evolves the multi-point propagator matrices for two-point correlations.
 * - **MT_MPPSigma**: Computes two-point correlation functions using MPP.
 * - **MT_MPP3**: Computes multi-point propagators for three-point correlations.
 * - **MT_MPPAlpha**: Computes three-point correlation functions using MPP.
 *
 * @section philosophy Philosophy and Usage
 * PyTransport 3.0 emphasizes the combination of flexibility, accessibility, and computational power.
 * Python serves as the primary interface for scripting and visualization, while the computationally
 * intensive tasks are handled in C++ to deliver near-native performance. This hybrid design allows
 * researchers to benefit from both ease of use and high numerical efficiency.
 *
 * @section license Licensing and Acknowledgments
 * PyTransport is free software: you can redistribute it and/or modify it under the terms of the
 * GNU General Public License as published by the Free Software Foundation, either version 3 of
 * the License, or (at your option) any later version. For more details, visit:
 ...
 *
 * @note This function expects two 1D NumPy arrays as input: `fieldsIn` for field values
 * and `params` for model parameters.
 * @retval None If the input array lengths do not match the expected number of fields/parameters,
 * an error message is printed and Python None is returned.
 */
 ...
 *
 * @note This function expects two 1D NumPy arrays: `fieldsIn` (field values)
 ...
 *
 * @note This function expects two 1D NumPy arrays: `fields_dfieldsIn` (concatenated
 * fields and velocities) and `params` (model parameters).
 */
 ...
 *
 * @note If `exit` is true, the evolution stops when the first slow-roll parameter
 * `epsilon` exceeds 1.
 */
