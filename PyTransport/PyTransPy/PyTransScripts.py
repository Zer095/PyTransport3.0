...
def pSpectraMPP(kA, back, params, NB, tols, MTE):
    ...
    for ii in range(0,num):
        ...
        if Nstart == np.nan:
            ...
            t=np.linspace(Nstart,back[-1,0], 10)
            # run solver for this triangle
            rho = MTE.MPP2(t, k,backExitMinus, params, tols)
...
def eqSpectra(kA, back, params, NB, tols, MTE):
    ...
    for ii in range(0,num):
        ...
        if Nstart == np.nan:
            nF = MTE.nF(); threePt = np.empty((2,5))
            threePt[:] = np.nan
        else:
...
def MPPSpectra(kA, back, params, NB, tols, MTE):
    ...
    for ii in range(0,num):
        ...
        if Nstart == np.nan:
            ...
            threePt = MTE.MPPAlpha(t,k1,k2,k3, backExitMinus,params,rho3, True) # all data from three point run goes into threePt array
        zzOut=np.append(zzOut, threePt[-1,1])
        zzzOut=np.append(zzzOut, threePt[-1,4])
        times = np.append(times, timeit.default_timer()-start_time)
...
# This script computes the k-mode that exit the horizon at a given N
...
# This script computes the k-mode that exit the horizon at a given Phi
...
def kexitN(Nexit, back, params, MTE, exact=False):
    ...
    for ii in range(0,    np.size(back[:,0])):
        Harr[ii]=MTE.H(back[ii,1:2*nF+1],params)
    logk = interpolate.splev(Nexit,interpolate.splrep(back[:,0], back[:,0] + np.log(Harr), s=1e-15),der=0)
